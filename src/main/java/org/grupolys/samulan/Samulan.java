...
public class Samulan {
	...
	private static Namespace parseCommand(String[] args) {
		...
		ap.addArgument("-f", "--" + FILTERING).required(false)
				.help("sentistrength to use SentiStrength output. Omit otherwise");
		...
		ap.addArgument("-v", "--" + VERBOSE).type(Boolean.class).setDefault(false).required(false)
				.help("Print explanation");
		ap.addArgument("-d", "--" + DEBUG).type(Boolean.class).setDefault(false).required(false)
				.help("Print explanation");
		...
		ap.addArgument("-spf", "--" + PATH_SAVED_PARSED_FILE).required(false).help(
				"Path to the file where to saved the parsed sentence in CoNLL format. Useful if you plan to run many experiments");
		Namespace ns = null;
		...
		try {
			...
			if (!ns.get(SCALE).equals(TRINARY) && !ns.get(SCALE).equals(SEMANTIC_ORIENTATION)
					&& !ns.get(SCALE).equals(BINARY)) {
				throw new ArgumentParserException(
						SCALE + " is not a valid argument. Use: " + SEMANTIC_ORIENTATION + "|" + TRINARY + "|" + BINARY,
						ap);
			}
	...
	// private static List<SentimentDependencyGraph> getGraphs(String path, String
	// encoding){
	// CoNLLReader conllReader = new CoNLLReader();
	// List<DependencyGraph> graphs =conllReader.read(path, encoding);
	// List<SentimentDependencyGraph> sgraphs = new
	// ArrayList<SentimentDependencyGraph>();
	//
	...
	private static void analyse(String conllFile, String encoding, RuleBasedAnalyser rba, String pathOutput,
			String scale, boolean verbose) {
			...
			while (line != null) {
				...
				if (newFileGraphs && (previousTextID != null)) {
					...
					String text = String.join(" ",
							sdgs.stream().map((SentimentDependencyGraph dg) -> dg.subgraphToString((short) 0))
									.collect(Collectors.toList()));
					...
					try {
						...
						writer.write(printOutputScaled(siFinal, scale, rba.getAc().isBinaryNeutralAsNegative()) + "\t"
								+ "\t" + text + "\t" + " [The analysis took: " + textAnalyseTime
								+ " seg.] [Accumulated time is: " + totalAnalyseTime + "]\n");
						// writer.write(previousTextID+"\t"+printOutputScaled(siFinal,scale,
						// rba.getAc().isBinaryNeutralAsNegative())+"\t"+"\t"+text+"\t"+" [The analysis
						// took: "+textAnalyseTime+" seg.] [Accumulated time is:
						// "+totalAnalyseTime+"]\n");
						writer.flush();
						if (verbose) {
			...
			if (!sdgs.isEmpty()) {
				...
				try {
					writer.write(printOutputScaled(siFinal, scale, rba.getAc().isBinaryNeutralAsNegative()) + "\t"
							+ "\t" + text + "\t" + " [The analysis took: " + textAnalyseTime
							+ " seg.] [Accumulated time is: " + totalAnalyseTime + "]\n");
					// writer.write(textID+"\t"+printOutputScaled(siFinal,scale,rba.getAc().isBinaryNeutralAsNegative())+"\t"+"\t"+text+"\t"+"
					// [The analysis took: "+textAnalyseTime+" seg.] [Accumulated time is:
					// "+totalAnalyseTime+"]\n");
					writer.flush();
					if (verbose) {
						sdgs.stream().forEach(sdg -> sdg.printLandscapeGraph((short) 0));
	...
	// Obtains the sentiment classification for line in a tsv file (it classifies
	// new samples)
	private static void analyse(String pathRawFiles, String encoding, Processor p, RuleBasedAnalyser rba,
			String pathOutput, String scale, boolean verbose, String pathToSaveParsedFile) {
		...
		try {
			...
			while (line != null) {
				...
				// If the user has provided a path to save the graphs in a conll file we save
				// them
				// if (pathToSaveParsedFile != null) {
				// conllTextId += 1;
				// conllWriter.write("### " + conllTextId + "\t" + ((ls.length > 1) ? ls[0] :
				// "NotAvailable") + "\n");
				// // System.out.println("### "+conllTextId+"\t"+((ls.length > 1)? ls[0] :
				// // "NotAvailable" )+"\n");
				...
				String sentiment = printOutputScaled(siFinal, scale, rba.getAc().isBinaryNeutralAsNegative());
				// writer.write("{ \"text\": \"" + text + "\", \"sentiment\": " + sentiment +
				// "},");

				JSONObject json = new JSONObject();
				JSONArray sentiAnalysisTree = new JSONArray();

				sdgs.stream().forEach(sdg -> {
					JSONObject sdgJson = sdg.toJson((short) 0);
					sentiAnalysisTree.add(sdgJson);
				});
				json.put("text", text);
				json.put("sentiment", sentiment);
				json.put("analysisTree", sentiAnalysisTree);
	...
	private static void execute(Namespace ns) {
		...
		if (sentiStrengthStyleOutput(ns.get(FILTERING))) {
			...
			return;
			// MaximumFromBranchSentimentJoiner msj = new
			// MaximumFromBranchSentimentJoiner();
			// // NormaliserSentimentJoiner nsj = new NormaliserSentimentJoiner(5);
			// CompositeSentimentJoiner csj = new CompositeSentimentJoiner(new
			// ArrayList<SentimentJoiner>(Arrays.asList(nsj,msj)));
		...
		if (parsedCoNLLFileProvided(ns)) {
			...
			analyse(ns.getString(CONLL), ns.getString(ENCODING), rba, ns.getString(OUTPUT), ns.getString(SCALE),
					ns.getBoolean(VERBOSE));
		} else {
			try {
				analyse(ns.getString(INPUT), ns.getString(ENCODING), configureProcessor(ns), rba, ns.getString(OUTPUT),
						ns.getString(SCALE), ns.getBoolean(VERBOSE), ns.getString(PATH_SAVED_PARSED_FILE));
